...
def query_line14_stations_locations():
    query = """
    SELECT name, latitude, longitude
    FROM `de-project-pulumi.visualization.line_14_stations_locations`
    """
    try:
        return client.query(query).to_dataframe()
    except Exception as e:
        st.error(f"Erreur lors du chargement des stations du métro 14 : {e}")
        return pd.DataFrame()
...
st.title("Metro Line 14: Geographical view")

df_line_stations = query_line14_stations_locations()
